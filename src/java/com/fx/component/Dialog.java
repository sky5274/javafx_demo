...
public class Dialog {
	public static void alert(String cont) {
		Alert alert = new Alert(AlertType.NONE,cont,new ButtonType("确认",ButtonData.YES));
		alert.setTitle("提示");
		alert.show();
	}
	
	public static <T> void comfrim(AlertAction<T> con) {
		Alert alert = new Alert(AlertType.CONFIRMATION,null,new ButtonType("确认",ButtonData.YES),new ButtonType("取消",ButtonData.CANCEL_CLOSE));
		alert.setTitle(StringUtils.isEmpty(con.getTitle())?"确认":con.getTitle());
